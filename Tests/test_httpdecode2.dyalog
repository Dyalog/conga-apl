...
⍝  Test http chunked transfere
 ...
 :For test :In (crlf Header Chunks Trailer)(crlf Header Chunks(0 2⍴''))(lf Header Chunks Trailer)(lf Header Chunks(0 2⍴''))
     ...
     :If (1009 'ERR_NAME_IN_USE')≡2↑ret←iConga.Clt clt Host Port'http'(4×size)('Options'iConga.DecodeHttp)
         'BlockSize'Log'Client still there delay and retry'
         ⎕DL 1
         ret←iConga.Clt clt Host Port'http'(4×size)('Options'iConga.DecodeHttp)
     :EndIf
     :If (0 clt)Check ret
